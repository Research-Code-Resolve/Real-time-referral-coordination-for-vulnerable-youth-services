...
function LoginScreen() {
  ...
  const handleRegister = async (event) => {
    ...
    if (!regName || !regEmail || !regPassword) {
      setRegError('Fill in all fields to request access.');
      return;
    }
    ...
    try {
      await new Promise((resolve) => setTimeout(resolve, 700));
      // Registration submitted — move to the pending-approval view.
      setView('pending');
    } catch {
      setRegError('We could not submit your request. Please try again.');
    } finally {
      setRegLoading(false);
    }
  };
        ...
        <div className="rounded-[36px] border border-white/70 bg-white/90 p-7 shadow-[0_25px_60px_-25px_rgba(15,23,42,0.18)] backdrop-blur-xl sm:p-8">
          <div className="mb-10 flex flex-col items-center gap-5 text-center">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500 sm:text-base">
                {view === 'login' && 'A clean, secure access point for your team.'}
                {view === 'register' && 'Request access to get started.'}
                {view === 'pending' && 'Your request has been received.'}
              </p>
            </div>
          </div>
          ...
          {view === 'register' && (
            <form className="space-y-4" onSubmit={handleRegister}>
              ...
              {regError && <p className="rounded-2xl bg-rose-50 px-4 py-3 text-sm text-rose-700">{regError}</p>}
          ...
          {view === 'pending' && (
            <div className="space-y-4 text-center">
              <div className="rounded-[28px] bg-slate-100 px-5 py-6 text-sm text-slate-600 shadow-sm">
                Thanks for registering. An administrator needs to approve your account before you can log in.
              </div>

              <div className="rounded-full bg-slate-100 px-4 py-3 text-center text-sm font-semibold text-slate-700 shadow-sm">
